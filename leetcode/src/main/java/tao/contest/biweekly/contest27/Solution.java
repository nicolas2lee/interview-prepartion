...
class Solution {
    public List<Boolean> checkIfPrerequisite(int n, int[][] prerequisites, int[][] queries) {
        ...
        for (int i=0; i<n; i++){
            ...
            mergeList(graph, graph.get(i), result);
            map.put(i, result);
        }

        List<Boolean> ans = new ArrayList<>();
        for (int i=0; i<queries.length;i++){
            Integer from = queries[i][1];
            ans.add(map.get(from).contains(queries[i][0]));
        }
        return ans;
    }

    private void mergeList(List<List<Integer>> graph, List<Integer> integerList, List<Integer> result) {
        result.addAll(integerList);
        for (int i=0; i<integerList.size();i++){
            mergeList(graph, graph.get(integerList.get(i)), result);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        //[[0,4],[4,0],[1,3],[3,0]]
        int[][] query = {{0,4}, {4,0}, {1,3},{3,0}};
        final List<Boolean> result = s.checkIfPrerequisite(5, graph, query);
